...
class FeatureConfig:
    ...
    # 支持同时加载多个 SSL 模型作为独立特征组
    # key = 自定义 group 名称（需加入 audio_features）, value = model_tag
    # 示例: {"ssl_wav2vec2xlsr": "wav2vec2-chinese-xlsr"}
    ssl_group_tags: dict[str, str] = field(default_factory=dict)
    # key = 自定义 group 名称（需加入 video_features）, value = model_tag
    # 示例: {"vision_ssl_clip": "clip-vit-large-patch14"}
    vision_ssl_group_tags: dict[str, str] = field(default_factory=dict)
...
class MultimodalDataset(Dataset):
    ...
    def _load_raw_groups(
        ...
        for feat_name in feat_list:
            tag: str | None = None
            fs_feat_name = feat_name  # 文件系统中的特征目录名
            if feat_name == "ssl_embed":
                tag = cfg.audio_ssl_model_tag
            elif feat_name == "vision_ssl_embed":
                tag = cfg.video_ssl_model_tag
            elif feat_name in cfg.ssl_group_tags:
                # 额外的音频 SSL 模型组：仍存储在 ssl_embed 目录下
                tag = cfg.ssl_group_tags[feat_name]
                fs_feat_name = "ssl_embed"
            elif feat_name in cfg.vision_ssl_group_tags:
                # 额外的视频 SSL 模型组：仍存储在 vision_ssl_embed 目录下
                tag = cfg.vision_ssl_group_tags[feat_name]
                fs_feat_name = "vision_ssl_embed"
            try:
                seq = load_sequence(
                    self.root, self.split,
                    str(row["anon_school"]), str(row["anon_class"]),
                    str(row["anon_pid"]),
                    modality, fs_feat_name, str(row["session"]),
                    model_tag=tag,
                )
